...
namespace TinyXRSDK
{
    ...
    /// iteself would define how to update the controller states, so that every frame TXRInput could get the right states.There
    /// are max two states for one controllerProvider.
    /// </summary>
    public partial class TXRInput : SingletonBehaviour<TXRInput>
    {
        ...
        private static TXRInput m_Instance = null;
        ...
        {
            if (m_ControllerProvider is TXRControllerProvider)
            {
                return ((TXRControllerProvider)m_ControllerProvider).GetVersion(index);
            }
            else
            {
                return "0.0.0";
        ...
        {
            // TODO
            // if (!TXREmulatorManager.Inited && !GameObject.Find("TXREmulatorManager"))
            // {
            //     TXREmulatorManager.Inited = true;
            //     GameObject.Instantiate(Resources.Load("Prefabs/TXREmulatorManager"));
            // }
            // if (!GameObject.Find("TXREmulatorController"))
            // {
            //     Instantiate(Resources.Load<GameObject>("Prefabs/TXREmulatorController"));
            // }
        }
