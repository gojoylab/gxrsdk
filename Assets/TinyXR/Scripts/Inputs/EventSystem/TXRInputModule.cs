...
namespace TinyXRSDK
{
    ...
    public class TXRInputModule : BaseInputModule
    {
        private int m_processedFrame;
        private static readonly List<TXRPointerRaycaster> raycasters = new List<TXRPointerRaycaster>();
        ...
        private static TXRInputModule m_Instance;
        public static TXRInputModule Instance
        {
            get
            {
                Initialize();
        ...
        protected virtual void ProcessPress(TXRPointerEventData eventData)
        {
        ...
        protected void ProcessPressDown(TXRPointerEventData eventData)
        {
            ...
            {
                if (eventData.raycaster != null && time < (eventData.clickTime + TXRInput.ClickInterval))
                {
        ...
        protected void ProcessPressUp(TXRPointerEventData eventData)
        {
            var currentOverGo = eventData.pointerCurrentRaycast.gameObject;
        ...
        protected bool ShouldStartDrag(TXRPointerEventData eventData)
        {
            ...
            var threshold = TXRInput.DragThreshold;
            return (currentPos - pressPos).sqrMagnitude >= threshold * threshold;
        }

        protected void ProcessDrag(TXRPointerEventData eventData)
        {
        ...
        protected static void HandlePressExitAndEnter(TXRPointerEventData eventData, GameObject newEnterTarget)
        {
            ...
            for (var t = oldTarget; t != null; t = t.parent)
            {
                ...
                {
                    ExecuteEvents.Execute(t.gameObject, eventData, TXRExecutePointerEvents.PressExitHandler);
                }
            }
            ...
            {
                ExecuteEvents.Execute(t.gameObject, eventData, TXRExecutePointerEvents.PressEnterHandler);
            }
        }
